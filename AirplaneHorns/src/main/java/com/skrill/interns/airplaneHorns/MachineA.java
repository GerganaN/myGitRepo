...
public class MachineA extends Machine {
    ...
    protected void load(Station station) {
        if (firstPart < REQUIRED_PART_A && station.getPartA() >= REQUIRED_PART_A) {
            ...
            airplaneHornsClient.sendToServer(REQUIRED_PART_A + " P_A are loaded in " + this.name + " from "
                    + station.getName());
        }
        ...
        if (this.secondPart < REQUIRED_PART_B && station.getPartB() >= REQUIRED_PART_B) {
            ...
            airplaneHornsClient.sendToServer(REQUIRED_PART_B + " P_B are loaded in " + this.name + " from "
                    + station.getName());
        }
    }
    ...
    protected void unload(Station station) {
        ...
        airplaneHornsClient.sendToServer(this.name + " produced " + PRODUCED_PART_D + " P_D");
    }
