...
public class MachineB extends Machine {
    ...
    protected void load(Station station) {
        if ((firstPart < REQUIRED_PART_C) && (station.getPartC() >= REQUIRED_PART_C)) {
            ...
            airplaneHornsClient.sendToServer(REQUIRED_PART_C + " P_C are loaded in " + this.name + " from "
                    + station.getName());
        }
        ...
        if ((this.secondPart < REQUIRED_PART_D) && (station.getPartD() >= REQUIRED_PART_D)) {
            ...
            airplaneHornsClient.sendToServer(REQUIRED_PART_D + " P_D are loaded in " + this.name + " from "
                    + station.getName());
    ...
    protected void unload(Station station) {
        ...
        airplaneHornsClient.sendToServer(this.name + " produced " + PRODUCED_PART_E + " P_E");
